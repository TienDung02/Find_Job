<div id="messages">
    @if($check_messages_prev == true)
        <div class="w-100 d-flex justify-content-center border-bottom hover-button mt-3 mb-3"><a class="w-100 text-center pb-2 fw-semibold add-messages-prev" data-page="{{$page}}" data-messages-id="{{$chat_selected->id}}">Previous Message</a></div>
    @endif
    @foreach($messages_selected as $key => $messages)
        ...
        @if($messages->sender_id != Session::get('user_data.id'))
            @if($key > 0 && $messages_selected[$key-1]->sender_id == $messages->sender_id)
                <div class="message-bubble d-flex width-55 mt-2">
            ...
            @else
                <div class="message-bubble d-flex width-55">
        ...
        @elseif($messages->sender_id == Session::get('user_data.id'))
            @if($key > 0 && $messages_selected[$key-1]->sender_id == $messages->sender_id)
                <div class="message-bubble mt-2 me d-flex justify-content-end">
                    <div class="message-text max-width-55 me-4 rounded-3">
