        ...
        <table class="table table-hover" id="table">
            ...
            @foreach($data as $key => $value)
                <tr>
                    ...
                    <td><input class="toggle_switch ms-3 mt-3" data-type="company with name " data-id="{{$value['id']}}" data-name="{{$value['company_name']}}" {{$value['active']==1?'checked':''}}  type="checkbox"></td>
                    <td>
    ...
    <div class="card-bottom ">
        <div class="paginate">
            @if ($data instanceof \Illuminate\Pagination\LengthAwarePaginator || $data instanceof \Illuminate\Pagination\Paginator)
                {{$data->withQueryString()->appends(request()->query())->links('.backend.component.paginate')}}
            @endif
        </div>
